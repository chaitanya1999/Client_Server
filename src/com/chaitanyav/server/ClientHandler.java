...
public class ClientHandler {    
    ...
    private long disconnectTimeout=Constants.CONN_CHK_TIMEOUT;   //default 10 seconds i.e. 10000ms
    private long pongTimeout=Constants.LATENCY;   //ping latency in ms
            ...
            while (ClientHandler.this.connected) {
                    if (isActive.get()) {       //If some data was received in the past disconnectTimeout ms
                        ...
                        try {
                            ...
                            sendDataEx(new Message(Constants.PING, "Are you alive?"));       //send a PING to check if the client is alive or not
                            synchronized (connCheckerLock) {
                                long startTime = System.nanoTime();
                                long curTime = startTime;
                ...
                try {
                    if(socket.getInputStream().available()>0){
                        ...
                        if(msg.getTag().equals(Constants.PING)){    //if client is checking connection
                            server.executeAction(sequentialExecutor,()->{sendData(new Message(Constants.PONG, "Connection is alive!"));}
                                    , true);
    ...
    public ClientHandler(Socket socket,Server server){
        ...
        try{
            this.server=server;
            disconnectTimeout=this.server.getDisconnectTimeout();
            pongTimeout=this.server.getLatency();
     ...
     * @throws IOException 
     */
    public final void sendDataEx(Message msg) throws IOException {
